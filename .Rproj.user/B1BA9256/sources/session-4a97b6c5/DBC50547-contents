# dalearningCCA
# CCA from Mikella modified with new and old ROIs 
# 4.30.23 KLS
...
d2r <- dt[c(2, 8, 14, 16, 18, 23, 27, 29, 17)]

#decision making
dm <- dt[3:7]

cca.fit <- cca(d2r, dm, xlab=c("AGE", "MIDBRAIN", "AMYGDALA", "INSULA", "ACC", "TEMPORAL", "PARIETAL", "FRONTAL", "THALAMUS"), ylab=c("INFLEX", "RISK_SEEK", "INCORR_CHOICE", "PROB_ERR_ABS", "PROB_ERR"), standardize.scores=TRUE)
...
write.csv(res1, here('output', 'altcca3_wilks.csv'))
